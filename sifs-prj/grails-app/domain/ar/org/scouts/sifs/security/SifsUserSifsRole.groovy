...
class SifsUserSifsRole implements Serializable {
	...
	boolean equals(other) {
		...
		other.sifsUser?.id == sifsUser?.id && other.sifsRole?.id == sifsRole?.id
	}
	...
	static SifsUserSifsRole get(long sifsUserId, long sifsRoleId) {
		find 'from SifsUserSifsRole where sifsUser.id=:sifsUserId and sifsRole.id=:sifsRoleId', [sifsUserId: sifsUserId, sifsRoleId: sifsRoleId]
	}
