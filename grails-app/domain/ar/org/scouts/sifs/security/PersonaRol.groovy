...
class PersonaRol implements Serializable {
	...
	boolean equals(other) {
		...
		other.persona?.id == persona?.id && other.rol?.id == rol?.id
	}
	...
	static PersonaRol get(long personaId, long rolId) {
		find 'from PersonaRol where persona.id=:personaId and rol.id=:rolId', [personaId: personaId, rolId: rolId]
	}

	static PersonaRol create(Persona persona, Rol rol, boolean flush = false) {
		def personaRol = new PersonaRol(persona: persona, rol: rol)
		personaRol.save(flush: flush, insert: true)
	}
